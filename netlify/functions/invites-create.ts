...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    await query(
      `insert into org_invites (id, organization_id, email, role, token, expires_at, created_by_user_id)
       values ($1,$2,$3,$4,$5,$6,$7)`,
      [randomUUID(), context.organization.id, body.email.toLowerCase(), body.role, token, expiresAt, context.user.id]
    );
