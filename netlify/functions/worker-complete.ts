...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const task = await one<{
      ...
      status: string;
    }>(`select organization_id, created_by_user_id, bot_id, cancellation_requested_at, status from tasks where id = $1`, [body.taskId]);
    ...
    if (body.status === 'awaiting_approval') {
      ...
      let approvalId = body.approval?.approval_id || randomUUID();
      await query(
