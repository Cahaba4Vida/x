...
export const handler = async (event) => {
    ...
    try {
        ...
        const task = await one(`select id, status, created_by_user_id
       from tasks
       where id = $1 and organization_id = $2`, [body.task_id, context.organization.id]);
        if (!task)
        ...
        if (task.status === 'queued') {
         ...
         set status = 'cancelled',
             ...
             completed_at = now(),
             updated_at = now()
         where id = $1`, [body.task_id, context.user.id, body.reason]);
            await query(`insert into task_events (task_id, event_type, payload)
         values ($1, 'cancelled', $2::jsonb)`, [body.task_id, JSON.stringify({ by: context.user.email, reason: body.reason, immediate: true })]);
       ...
       where id = $1`, [body.task_id, context.user.id, body.reason]);
        await query(`insert into task_events (task_id, event_type, payload)
       values ($1, 'cancellation_requested', $2::jsonb)`, [body.task_id, JSON.stringify({ by: context.user.email, reason: body.reason })]);
