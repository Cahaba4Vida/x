...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const existing = await one<{ id: string }>(`select id from users where email = $1`, [body.email.toLowerCase()]);
    ...
    if (body.invite_token) {
      const invite = await one<{
        ...
        `select organization_id, role, email, expires_at, used_at from org_invites where token = $1`,
        [body.invite_token]
      );

      if (!invite) return badRequest('Invite not found');
      if (invite.used_at) return badRequest('Invite already used');
      if (new Date(invite.expires_at).getTime() < Date.now()) return badRequest('Invite expired');
      ...
      await query(
        `insert into users (id, email, password_hash, full_name, default_org_id)
         values ($1,$2,$3,$4,$5)`,
        [userId, body.email.toLowerCase(), passwordHash, body.full_name, orgId]
      );
