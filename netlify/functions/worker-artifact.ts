...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    await query(
      `insert into task_artifacts (id, task_id, run_id, artifact_type, name, storage_url, metadata)
       values ($1,$2,$3,$4,$5,$6,$7::jsonb)`,
      [artifactId, body.taskId, body.runId ?? null, body.artifactType, body.name ?? null, body.storageUrl, JSON.stringify({ ...body.metadata, worker_id: workerId })]
    );

    await query(
      `insert into task_events (task_id, run_id, event_type, payload)
       values ($1,$2,'artifact_added',$3::jsonb)`,
      [body.taskId, body.runId ?? null, JSON.stringify({ artifact_id: artifactId, artifact_type: body.artifactType, name: body.name ?? null, storage_url: body.storageUrl, worker_id: workerId })]
    );
