...
export const handler = async (event) => {
    ...
    if (!context)
        return unauthorized();
    const bots = await query(`select id, name, status, last_heartbeat_at, created_at
     from bots where organization_id = $1 order by created_at asc`, [context.organization.id]);
