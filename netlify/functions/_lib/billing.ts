...
export function estimateUnitsFromPrompt(prompt: string): number {
  return Math.max(1, Math.ceil(prompt.length / 150) / 10);
}

export function calculateBillableUnits(input: UsageInput): number {
...
export async function recordUsageForTask(args: {
  ...
  runId: string;
  usage: UsageInput;
}): Promise<{ usageLedgerId: string; billableUnits: number }> {
  const id = randomUUID();
...
export async function syncUsageToStripe(args: { usageLedgerId: string; userId: string; billableUnits: number }) {
  const customer = await one<{ stripe_customer_id: string | null; stripe_subscription_status: string | null }>(
    `select stripe_customer_id, stripe_subscription_status from stripe_customers where user_id = $1`,
    [args.userId]
  );
  ...
  await query(
    `insert into billing_events (id, user_id, usage_ledger_id, stripe_customer_id, meter_event_identifier, status)
     values ($1,$2,$3,$4,$5,'pending')
     on conflict (meter_event_identifier) do nothing`,
    [randomUUID(), args.userId, args.usageLedgerId, customer.stripe_customer_id, identifier]
  );
  ...
  if (!response.ok) {
    ...
    await query(
      `update billing_events set status = 'failed', error_message = $2 where meter_event_identifier = $1`,
      [identifier, text.slice(0, 1000)]
    );
    return { sent: false, reason: text };
  }

  await query(
    `update billing_events set status = 'sent', sent_at = now() where meter_event_identifier = $1`,
    [identifier]
  );
