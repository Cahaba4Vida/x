...
export const handler = async (event) => {
    ...
    let mapping = await one(`select stripe_customer_id from stripe_customers where user_id = $1`, [context.user.id]);
    ...
    if (!customerId) {
        ...
        await query(`insert into stripe_customers (user_id, stripe_customer_id, billing_mode)
       values ($1,$2,'metered')
       on conflict (user_id) do update set stripe_customer_id = excluded.stripe_customer_id, updated_at = now()`, [context.user.id, customerId]);
    }
