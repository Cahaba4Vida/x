...
export const handler = async (event) => {
    ...
    try {
        ...
        await query(`update bots
       set status = $2,
           machine_name = coalesce($3, machine_name),
           last_heartbeat_at = now(),
           updated_at = now()
       where id = $1`, [body.botId, body.status, body.machineName ?? null]);
        await query(`insert into worker_leases (bot_id, lease_owner, lease_expires_at, heartbeat_at, created_at, updated_at)
       ...
       on conflict (bot_id) do update set
         ...
         updated_at = now()`, [body.botId, workerId, String(leaseSeconds)]);
