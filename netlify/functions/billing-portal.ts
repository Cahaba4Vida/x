...
export const handler: Handler = async (event) => {
  ...
  if (!baseUrl) return badRequest('Missing APP_BASE_URL');

  const mapping = await one<{ stripe_customer_id: string | null }>(
    `select stripe_customer_id from stripe_customers where user_id = $1`,
    [context.user.id]
  );
  if (!mapping?.stripe_customer_id) return badRequest('Stripe customer not found');
